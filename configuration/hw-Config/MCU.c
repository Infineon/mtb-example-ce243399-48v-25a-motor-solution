...
void MCU_RunISR0(void)
{
    ...
    //----------------------------------------------------------------------------------
    //--- Position sensor SPI readout and acquisition.
    //----------------------------------------------------------------------------------
    #if defined(ENC_IN_SPI_MODE)
        #if defined (ENC_TLx5012)
            // Store new sensor raw value
            uint32_t theta_raw = TLx_5012B_POS_RAW;
            // Update the double integrator filter
            BiIntFilt_Update(&BiIntFilt_obj, theta_raw, motor[0].sm_ptr->current == Align, Cy_TCPWM_Counter_GetCounter(TIMER_250US_HW, TIMER_250US_NUM) / 4u);
            // Request new value from sensor
            Rqst_TLx_5012B_POS(CY_SCB_SPI_SLAVE_SELECT0);
        #endif
    
        // Pass position and speed to motor control library
        if (motor[0].params_ptr->sys.fb.mode == Direct){
            motor[0].vars_ptr->w_fb.elec    = BiIntFilt_obj.omega_elec_flt;
            motor[0].vars_ptr->th_r_fb.elec = BiIntFilt_obj.theta_elec_flt;
            motor[0].vars_ptr->th_r_fb.mech = BiIntFilt_obj.theta_mech_flt;
        }
    #endif
    //----------------------------------------------------------------------------------
    ...
    #if defined(BOARD_CS_3X_IGJ022H10SA2)
        // Default code from V3.2
    ...
    #else
        // Adaption for 2 shunt measurement (U & W where W is inverted)
        float samp_0 = mcu[0].adc_scale.i_uvw * (Curr_ADC_Half_Point_Ticks - (uint16_t)mcu[0].dma_results[mcu[0].adc_mux.idx_isamp[0]])*motor[0].params_ptr->sys.analog.shunt.current_sense_polarity;
        float samp_2 = mcu[0].adc_scale.i_uvw * (Curr_ADC_Half_Point_Ticks - (uint16_t)mcu[0].dma_results[mcu[0].adc_mux.idx_isamp[2]])*motor[0].params_ptr->sys.analog.shunt.current_sense_polarity;
        motor[0].sensor_iface_ptr->i_samp_0.raw = samp_0;
        motor[0].sensor_iface_ptr->i_samp_1.raw = samp_2 - samp_0;
        motor[0].sensor_iface_ptr->i_samp_2.raw = -samp_2;
    #endif
...
{
    // Disable 7th switch
    Cy_GPIO_Write(PROTECTION_SW_PORT, PROTECTION_SW_PIN, 0);
...
{
    // Enable 7th switch
    Cy_GPIO_Write(PROTECTION_SW_PORT, PROTECTION_SW_PIN, 1);
