...
/*****************************************************************/
/******************Motor and Board Setup**************************/
/*****************************************************************/
/* Define Board Setup */
#define BOARD_CS_2X_TLE5571             // 2x current sense on U/W with the TMR TLE5571 sensors (default). The third signal will be calculated
//#define BOARD_CS_3X_IGJ022H10SA2      // 3x current measurement with the integrated current sensing og the iGaN IGJ022H10SA2. Hardware must be changed for this to work. See user guide

/* Define Encoder protocol */
//#define ENC_IN_IIF_MODE                 // In this mode the IIF encoder signals are used together with Motif (andromeda default). Note that this mode is not ideal for position control and leads to instability over time. 
#define ENC_IN_SPI_MODE                 // Use the encoder in SPI mode. The angle value is read via DMA and filtered with BiInfFilt to get the mech/elec angle and omega. ONE ENCODER MUST BE SELECTED!

/* Define used Encoder */
#define ENC_TLx5012                       // In this mode the TLx5012 encoder is used in SPI mode. The angle value is read via DMA and filtered with BiIntFilt to get the mech/elec angle and omega

/*****************************************************************/
/********Angle and Speed Filter (Double Integrator)***************/
/*****************************************************************/
#define BIINTFILT_L1                             (0.5f)                        // k_L1 mechanical
#define BIINTFILT_L2                             (100.0f)                      // k_L2 mechanical
#define BIINTFILT_SPEED_LIMIT                    (5000.0f)                     // omega_limit (rad/s). Acts as anti-windup
#define BIINTFILT_DEADBAND                       (0.0004f)                     // Deadband (rad) [unused]
#define BIINTFILT_FIXED_OFFSET_MECH              (1621u)                       // Magnet to shaft offset [unused]
...
#define ADC_CS_SHUNT_TYPE                        (Three_Shunt)                 /*Three_Shunt =0, Single_Shunt =1, Two_Shunt =2*/

#define ADC_CS_SHUNT_RES                         (1.0E-4f)                     /*[Ohm], Applicable for Current shunt type of measurement*/

#define ADC_CS_CURRENT_SENSITIVITY               (1.0E-3f)                     /*[V/A], Applicable for active current sense type of measurement*/

#if defined(BOARD_CS_2X_TLE5571)
    #define ADC_CS_OPAMP_GAIN                        (20.0f)                       /*[V/V], external amplifier gain for current measurement*/
#elif defined(BOARD_CS_3X_IGJ022H10SA2)
    #define ADC_CS_OPAMP_GAIN                        (26.1f)                       /*[V/V], external amplifier gain for current measurement*/
#endif

#define ADC_CS_SETTLE_RATIO                      (0.8f)                        /*[], setTLxng ratio used for single-shunt current sampling*/
...
#define ADC_SCALE_VDC                            (0.0519333333f)                /*[V/V] = [Ohm/Ohm]*/
/*******************************************************************************/
/*******************************************************************************/
/*Parameter Controls*/
/***MCU***/
...
#define MOTOR_CTRL_FASTLOOP_FREQ                   (15000.0f)                  /*[Hz], fast-loop frequency at least 1.2 decade above maximum electrical frequency, 1.5kHz~>25kHz*/
#define MOTOR_CTRL_FS0_FS1_RATIO                   (5U)                        /*[], Fast-loop to slow-loop frequency ratio*/

#define MOTOR_CTRL_PWM_DEADTIME                    (0.022E-6f)                 /*[sec], PWM Deadtime value*/
...
#define MOTOR_OFFSET_CURR_MIN                      (1.485f)                     /*[V], minimum voltage offset*/
#define MOTOR_OFFSET_CURR_MAX                      (1.815f)                     /*[V], maximum voltage offset*/
/*********Command*********/
...
#define MOTOR_CTRL_VOLT_MOD_SCHEME                 (Neutral_Point_Modulation)  /*Neutral_Point_Modulation =0,Space_Vector_Modulation =1*/
...
#endif
/*********Rotor Pre-Alignment*********/
#define MOTOR_CTRL_ALIGN_TIME                      (1.75f)                      /*[sec], Alignment time*/
